...
export function RightSidebar() {
  return (
    <aside className="hidden xl:block w-[300px] shrink-0 space-y-4 py-4 pl-6">
      {/* Welcome / login card */}
      <div className="border border-warning/40 bg-warning/10 rounded-md overflow-hidden">
        <div className="px-4 py-3 border-b border-warning/30 bg-warning/15 text-[13px] font-semibold">
          🎓 Chào mừng đến Universe
        </div>
        <div className="p-4 text-[13px] space-y-2">
          <p>Tham gia cộng đồng sinh viên — chia sẻ tài liệu, đọc review, sống sót qua đại học.</p>
          ...
          <button className="mt-2 w-full h-8 text-[13px] font-medium bg-primary text-primary-foreground rounded hover:brightness-110">
            Tham gia ngay
          </button>
        </div>
      </div>
      ...
      <div className="border border-border bg-surface rounded-md overflow-hidden">
        <div className="px-4 py-2 border-b border-border bg-muted text-[13px] font-semibold flex items-center gap-2">
          <Flame className="h-4 w-4 text-primary" /> Bài hot tuần này
        </div>
        <ul className="text-[13px] divide-y divide-border">
          {[
            ...
            <li key={i} className="px-4 py-2 hover:bg-muted">
              <a href="#" className="flex gap-2">
                <span className="text-[12px] font-mono text-success min-w-[28px] text-right">{it.v}</span>
                <span className="text-accent-foreground hover:text-primary">{it.t}</span>
              </a>
            </li>
          ))}
        </ul>
      ...
      <div className="border border-border bg-surface rounded-md overflow-hidden">
        ...
        <div className="p-4 grid grid-cols-2 gap-3 text-[12px]">
          ...
          <Stat icon={<Flame className="h-4 w-4 text-destructive" />} v="2,103" l="Nhang đã thắp" />
      ...
      {/* Top contributors */}
      <div className="border border-border bg-surface rounded-md overflow-hidden">
        <div className="px-4 py-2 border-b border-border bg-muted text-[13px] font-semibold">
          🏆 Top đóng góp tuần
        </div>
        <ul className="text-[13px]">
          {[
            ...
            <li key={i} className="flex items-center gap-2 px-4 py-2 hover:bg-muted">
              <span className="w-5 text-center">{m}</span>
              <div className="h-6 w-6 rounded bg-gradient-to-br from-primary to-accent" />
              <a href="#" className="flex-1 text-accent-foreground hover:text-primary truncate">{n}</a>
